...
.service("Asset", function($rootScope, $http, $routeParams, EngineDriver)
{
    ...
    if ($routeParams.projectId)
    {
      $http.get("/data/projects/" + $routeParams.projectId + "/assets").success(function(assetList) {
        assetList.forEach(function(assetId) {
          ...
          self.loadAsset(assetId, function(err)
          {
            if (err) throw err
  ...
  this.updateAsset = function(assetId, data, callback)
  {
    $http.put("/data/projects/" + $routeParams.projectId + "/assets/" + assetId, {assetData: data}).
      success(function()
      {
        ...
        self.unloadAssetInEngine(assetId, staticPath, function(err)
        {
          if (err) throw err
          self.loadAssetInEngine(assetId, staticPath, data, callback)
        })
      }).
      error(function(error)
      {
  ...
  this.loadAsset = function(assetId, callback)
  {
    ...
    $http.get("/data/projects/" + $routeParams.projectId + "/assets/" + assetId, {transformResponse: function(data) {return data;} })
      .success(function(data) {
        ...
        return self.loadAssetInEngine(assetId, staticPath, data, callback)
      })
      .error(callback);
  }
  ...
  this.unloadAsset = function(assetId)
  {
    ...
    return self.unloadAssetInEngine(assetId, staticPath, function() {})
  }

  this.loadAssetInEngine = function(assetId, staticPath, data, callback)
  {
    ...
    return EngineDriver[assetType.loader](options, callback);
  }

  this.unloadAssetInEngine = function(assetId, staticPath, callback)
  {
    var parts = assetParts(assetId);
    var assetType = assetTypes[parts.type];
    if (!assetType) {
    ...
    return EngineDriver[assetType.unloader](options, callback);
  }
